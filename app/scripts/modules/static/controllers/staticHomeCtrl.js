...
angular
    .module('static')
    .controller('staticHomeCtrl', function ($scope, $rootScope, $location) {
        $rootScope.listPost = [
            /*{
                'id': '',
                'title': '',
        ...
        $scope.loadMore = function() {
            $rootScope.listPost = $scope.listPost.concat(newPosts);
        }

        $scope.searchPosts = function() {
            $rootScope.listPost = newPosts;
            var changeURL = $location.replace("/search/1");
        }
    });
